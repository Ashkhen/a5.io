...
Muse.Utils.firstDescendant = function (a, b, c) {
    for (var d = 0; d < a.childNodes.length; d++) {
        ...
        if (f.nodeType == 1) {
            if (!b || b.matches(f))return f;
            if (!c || !c.matches(f))if (f = Muse.Utils.firstDescendant(f, b, c))return f
        }
    }
    return null
};
Muse.Utils.descendants = function (a, b, c, d) {
    ...
    for (var f = 0; f < a.childNodes.length; f++) {
        ...
        h.nodeType == 1 && ((!b || b.matches(h)) && d.push(h), (!c || !c.matches(h)) && Muse.Utils.descendants(h, b, c, d))
    }
    return d
};
...
Muse.Utils.anyStyleSheetRule = function (a, b) {
    var c = !1, d = a.cssRules;
    if (!d)c = !0, d = a.rules;
    if (!d)return null;
    for (var f = 0; f < d.length; ++f) {
        var h = d[f];
        if (Muse.Utils.isDefined(h.selectorText))if (c) {
            if (b(h.selectorText))return h
        } else for (var j = h.selectorText.split(/\s*,\s*/), g = 0; g < j.length; g++)if (b(j[g]))return h
    }
    return null
};
...
Muse.Utils.cloneStyleSheetRule = function (a) {
    ...
    var b = Muse.Utils.getPageStyleSheet(a), c = Muse.Utils.getStyleSheetRuleById(b, a), d = "c" + Muse.Utils.cloneStyleSheetRule.newNumber++, f = "#" + d;
    b.insertRule ? b.insertRule(c.cssText.replace("#" + a, f), b.cssRules.length) : b.addRule(f, c.style.cssText);
    return d
};
...
Muse.Utils.processHyperlink = function (a) {
    ...
    if (!d || d == "_self") {
        var f = b.lastIndexOf("/"), d = b.lastIndexOf("#"), h = a.attr("class").match(/anim_(\w+)/);
        if (h && d > f) {
            var a = c.data("scrollWrapper"), j = b.substring(d), d = $(j).offset(), b = h[1], g = a || window, f = document.documentElement || document.body, h = (a ? a.scrollHeight() : f.scrollHeight) - c.height(), c = (a ? a.scrollWidth() : f.scrollWidth) - c.width(), k = Math.min(h, d.top + (a && !a.isStandard() ? a.scrollTop() : 0)), i = Math.min(c, d.left +
                (a && !a.isStandard() ? a.scrollLeft() : 0)), c = function () {
                g.scrollTo(i, k);
                try {
                    history.replaceState({})
                } catch (a) {
                    if (!jQuery.browser.msie || jQuery.browser.version > 7)window.location.hash = j
                }
            };
            try {
            ...
            if (window.scrollTo || void 0 !== a) {
                var a = a || $(document), l = a.scrollLeft(), p = a.scrollTop(), q = l, o = p;
                $({scrollDistance: 0}).animate({scrollDistance: 1}, {duration: 1E3, easing: b, step: function (a) {
                    a != 0 && (o = a * (k - p), q = a * (i - l), g.scrollTo(l + q, p + o))
                }, complete: c})
            } else $("html,body").animate({scrollTop: k,
                scrollLeft: i}, 1E3, b, c);
            return!1
        }
    }
...
Muse.Utils.redirectHyperlink = function (a) {
    if (Muse.Utils.redirectCancelled)setTimeout(function () {
        Muse.Utils.redirectCancelled = !1
    }, 0); else if (actionStack = [], Muse.Utils.processHyperlink(a)) {
        var b = $(a).attr("target");
        b || (b = "_self");
...
Muse.Utils.enableAnchorLinksActiveState = function () {
    var a = $("#page"), b = a.outerWidth() / a.outerHeight() > 2, c = [], a = $(window), d = Muse.Utils.getPageStyleSheet(), f = function () {
        c.splice(0, c.length);
        $("a.nonblock,a.block").each(function () {
            Muse.Utils.saveHyperlinkInfo($(this), d, b, c)
        });
        c.sort(function (a, b) {
    ...
    f();
    if (0 != c.length) {
        var h = !1, j = a.data("scrollWrapper"), g = j || a, k = function () {
            h = !1;
            var a = b ? g.scrollLeft() : g.scrollTop(), d;
            a:{
                var i = 0;
                d = c.length;
                for (var k; i <
                    d; i++)if (k = c[i], k.from <= a && a <= k.to) {
                    d = i;
                    break a
                }
                d = -1
            }
            var o, n, i = Math.max(0, d);
            for (d = Math.min(d + 2, c.length); i < d; i++)if (o = c[i], k = o.$elem.offset().left + (j && !j.isStandard() ? j.scrollLeft() : 0), n = o.$elem.offset().top + (j && !j.isStandard() ? j.scrollTop() : 0), o.from != (b ? k : n)) {
                f();
                break
            }
            i = 0;
            for (d = c.length; i < d; i++) {
                o = c[i];
                k = o.from <= a && a <= o.to;
                o = o.hyperLinks;
                n = void 0;
                for (var s = 0; s < o.length; s++)n = o[s].hasClass("MenuItem") ? "MuseMenuActive" : "MuseLinkActive", k && !o[s].hasClass(n) ? o[s].addClass(n) : !k && o[s].hasClass(n) &&
                    o[s].removeClass(n)
            }
        }, i = function () {
            h || (h = !0, Muse.Utils.requestAnimationFrame(k))
        };
        if (a.data("scrollWrapper"))a.on("scrolled", i); else a.scroll(i);
        k()
    }
};
Muse.Utils.saveHyperlinkInfo = function (a, b, c, d) {
    var f = a.attr("href"), h = Muse.Utils.pathOnly(f), j = -1, g = a.attr("target"), k = window.location.href.replace(/#.*$/i, "");
    if (f && -1 != f.indexOf("#") && !(g && g != "_self") && !(0 <= h.indexOf("/")) && (k.charAt(k.length - 1) == "/" && (k += "index.html"), -1 != k.indexOf("/" + h, k.length - h.length - 1))) {
        var g = $(window).data("scrollWrapper"), k = f.substring(f.lastIndexOf("#")), a = a.parent('[class~="sbg"]').length > 0 || a.hasClass("block") ? a.parent() : a, i = a.attr("id");
        if (null !== Muse.Utils.anyStyleSheetRule(b,
            function (a) {
                return 0 <= a.indexOf("#" + i + ".MuseMenuActive") || 0 <= a.indexOf("#" + i + ".MuseLinkActive") || 0 <= a.indexOf(".MuseMenuActive#" + i) || 0 <= a.indexOf(".MuseLinkActive#" + i)
            })) {
            b = 0;
            for (h = d.length; b < h; b++)if (d[b].href == f) {
                j = b;
                break
            }
            if (-1 == j) {
                ...
                c = c ? k.offset().left + (g && !g.isStandard() ? g.scrollLeft() : 0) : k.offset().top + (g && !g.isStandard() ? g.scrollTop() : 0);
                g = Number.MAX_VALUE;
                b = 0;
                for (h = d.length; b < h; b++)if (d[b].href != f && d[b].from == c) {
                    j = b;
                    break
                }
                if (-1 == j) {
                    ...
                    d.push({hyperLinks: [], from: c, to: g, $elem: k, href: f});
                    j = d.length - 1
                }
            }
            d[j].hyperLinks.push(a)
        }
    }
};
...
Muse.Utils.needPIE = function (a) {
    if (Muse.Utils.havePIE)a(); else if (Muse.Utils.pieFunctionQueue.push(a), !Muse.Utils.pieLoading)Muse.Utils.pieLoading = !0, a = "scripts/pie.js", a[0] == "/" && (a = location.pathname.indexOf(".html") != -1 ? location.pathname.substring(0, location.pathname.lastIndexOf("/")) + a : location.pathname + a, a = a.replace(/\/+/g, "/")), $.ajax({url: a, dataType: "script", complete: function () {
...
Muse.Utils.transformMarkupToFixBrowserProblemsPreInit = function () {
    jQuery.browser.msie ? (jQuery("html").addClass("ie"), jQuery.browser.version < 8 && Muse.Utils.changeLItoDIVs()) : jQuery.browser.SafariMobile && jQuery("body").css("-webkit-text-size-adjust", "none")
};
...
Muse.Utils.applyIEFilterToPNGImages = function () {
    jQuery.browser.msie && jQuery.browser.version < 9 && $("body *").not(".museBgSizePolyfill img,.f3s_top,.f3s_mid,.f3s_bot").each(function () {
        var a = $(this);
        if (a.css("background-image").match(/\b.png/i) || this.nodeName && this.nodeName.toLowerCase() == "img" && a.attr("src").match(/\b.png/i)) {
            var b = a.css("filter");
            ...
            if (!b.data("mu-ie-matrix")) {
                ...
                if (!c || !(c.toLowerCase().indexOf("opacity") > 0 && c.indexOf("=100") < 0)) {
                    if (a.childNodes.length && !Muse.Browser.Features.checkCSSFeature("border-radius") && (c = Muse.Utils.firstChild(a)) && c.nodeName.toLowerCase() == "img") {
                        var c = $(c), d = c.attr("src"), f = b.css("background-color") + " ", h = c.css("margin-left");
                        if (h == "0px" || h == "auto")h = c.css("padding-left");
                        var j = c.css("margin-top");
                        if (j == "0px" || j == "auto")j = c.css("padding-top");
                        c.css("visibility", "hidden");
                        b.css("background", f + "url(" + d + ") no-repeat " + h + " " + j)
                    }
                    if (jQuery.browser.msie && jQuery.browser.version < 8 && (b.css("border-left-width") == 0 || b.css("border-left-style") == "none") && (b.css("border-right-width") == 0 || b.css("border-right-style") == "none") && (b.css("border-top-width") == 0 || b.css("border-top-style") == "none") && (b.css("border-bottom-width") == 0 || b.css("border-bottom-style") == "none"))b.css({"border-right-width": "1px",
                        "border-right-style": "solid", "border-right-color": b.css("background-color")}), b.width(b.width() - 1);
                    PIE.attach(a)
                }
            }
        })
...
Muse.Utils.initializeAnimations = function (a) {
    ...
    $(".animationContainer").each(function () {
        var a = $(this);
        this.contentDocument && "complete" == this.contentDocument.readyState ? b(a) : a.load(function () {
            b(a)
        })
    })
};
Muse.Utils.fixTransformRotations = function () {
    Muse.Browser.Features.checkCSSFeature("transform") || $("*[data-mu-ie-matrix]").each(function () {
        var a = $(this), b = a.parent(), c = Math.round(a.data("mu-ie-matrix-dx")), d = Math.round(a.data("mu-ie-matrix-dy")), f = b.innerHeight(), h = b.innerWidth();
        a.css({filter: function (b, c) {
            if (c)return c + " " + a.data("mu-ie-matrix");
            return a.data("mu-ie-matrix")
        }, "margin-bottom": "-=" + d}).removeClass("shadow");
        b.css({"margin-bottom": "-=" + (b.innerHeight() - f), "margin-right": "-=" + (b.innerWidth() -
            h)});
...
Muse.Utils.fullPage = function (a) {
    var b = $(a + " .verticalspacer"), c = $(a), d = $(window), f = $("body");
    b.css("min-height", 1);
    b.height() < 1 && b.height(Math.floor(2));
    var h = b.height();
    f.bind("wp-page-height-change", function (a, b) {
        q(b)
    });
    var j = Muse.Utils.getCSSIntValue(f, "padding-top") + Muse.Utils.getCSSIntValue(f, "margin-top"), g = Muse.Utils.getCSSIntValue(f, "padding-bottom") + Muse.Utils.getCSSIntValue(f, "margin-bottom"), k = !0, i = f.hasClass("always_vert_scroll"), m = 0 != Muse.Utils.getCSSIntValue(b, "margin-bottom"), l = function (a) {
        var a =
            1 < Math.floor(a * 100) / 100, c = !1;
        i || (a && !f.hasClass("no_vert_scroll") ? (f.addClass("no_vert_scroll"), c = !0) : !a && f.hasClass("no_vert_scroll") && (f.removeClass("no_vert_scroll"), c = !0));
        c && b.css("height")
    }, p = function (a) {
        parseInt(a) || (a = 0);
        m && b.css("margin-bottom", -(b.offset().top - j));
        var f = c.outerHeight(!0), i = f - h, a = Math.max(1, d.height() - j - g - i - a);
        a != h && (k = !1, l(a), b.css("height", a), a < h && f == c.outerHeight(!0) && (a = h, l(a), b.css("height", a)), k = !0);
        return h = a
    }, q = function (a) {
        k && p(a)
    };
    Muse.Browser.Bugs.CannotHandleClearBoth &&
        0 != Muse.Utils.getCSSIntValue(b, "margin-bottom") && (b.css("margin-bottom", 0), m = !1);
    (function () {
...
Muse.Utils.detachIframesAndObjectsToPauseMedia = function (a) {
    ...
    $("iframe, object", a).each(function () {
        ...
        if (!a.is("object") || !(jQuery.browser.msie && jQuery.browser.version < 9)) {
            ...
            d.$next = a.next();
            d.$parent = a.parent();
            jQuery.browser.msie ? (d.html = a.wrap("<div id='deleteMeWrapper'/>").parent().html(), a.remove(), d.$parent.children("div #deleteMeWrapper").remove()) : d.$node = a.detach();
            b.push(d)
        }
    });
    ...
    $("video", a).each(function () {
        if (jQuery.browser.msie && jQuery.browser.version ==
            9 && this.pause && this.getAttribute("autoplay") && this.readyState != 4)$(this).one("play", function () {
            this.pause()
...
Muse.Browser.Features.Touch = function () {
    if (navigator.maxTouchPoints > 0)return{Start: "pointerDown", End: "pointerUp", Move: "pointerMove", Listener: function (a) {
        return function (b) {
        ...
        if (c + "MaxTouchPoints"in navigator && navigator[c + "MaxTouchPoints"])return c = c.toUpperCase(), {Start: c + "PointerDown", End: c + "PointerUp", Move: c +
    ...
    try {
        if (document.createEvent("TouchEvent"), "ontouchstart"in window)return{Start: "touchstart", End: "touchend", Move: "touchmove", Listener: function (a) {
            return a
        }}
    } catch (d) {
    }
...
Muse.Browser.Features.checkCSSFeature = function (a, b) {
    ...
    for (var c = c.charAt(0).toUpperCase() + c.substr(1), d = 0, f = Muse.Browser.domPrefixes.length; d < f; d++)if (Muse.Browser.domPrefixes[d] + c in b.style)return Muse.Browser.domPrefixes[d];
    return!1
};
Muse.Browser.Features.checkCSSValueCompatibility = function (a, b) {
    ...
    for (var f = 0; f < Muse.Browser.domPrefixes.length; f++) {
        var h = "-" + Muse.Browser.domPrefixes[f].toLowerCase() + "-" + b;
        c.style[a] = h;
        if (c.style[a] !== d)return Muse.Browser.domPrefixes[f]
    }
    return!1
};
Muse.Browser.Bugs = {};
